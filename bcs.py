...
def parsing_ria(http,seek_list):
    ...
    last_news=now_site.find_all(category['bcs']['text']['box'],class_='feed-item__head')
        ...
        for j in text.split():
            for seek in seek_list:
                if seek in j:
                    ...
                    all_needs_news.append(category['bcs']['link']+i.get('href'))
...
for name in category['bcs']['http'].items():
